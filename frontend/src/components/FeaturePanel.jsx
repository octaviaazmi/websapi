      ...
      >
        {features.map((f) => (
          <motion.div
            key={f.title}
            variants={item}
            className="bg-gradient-to-br from-amber-400 to-orange-400 rounded-2xl px-5 py-6 flex flex-col items-center text-center gap-3 shadow-xl border border-amber-300/50 hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 cursor-default"
          >
            <div className="w-14 h-14 rounded-2xl bg-white/30 flex items-center justify-center text-3xl shadow-inner border border-white/30">
              {f.icon}
            </div>
            <h3 className="font-extrabold text-white text-base tracking-tight leading-tight drop-shadow">
              {f.title}
            </h3>
